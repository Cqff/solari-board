...
一面 10 行的 TPE 出境看板，可全螢幕上牆。照台北時間顯示**前 1 小時到後 4 小時**
的班次，狀態跟著時鐘自己翻；裝不下的班次每 25 秒翻下一頁，翻完回到最前面。
...
看板只把「前 1 小時到後 4 小時」那一段畫出來。班表有兩個來源：`tools/serve.mjs` 跑起來
...
分段在 `STATES`。視窗前後長度分別是 `BACK_MIN`（60 分）和 `WINDOW_MIN`（240 分）
—— 前一小時那段全部是 `DEPARTED`，留著讓人確認自己是不是錯過了。視窗會繞過午夜：
