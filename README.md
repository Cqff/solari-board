...
> **⚠ 這個來源目前是死的。** 資料集頁面寫「每 5 分更新」，但實際下載到的是
> **2023-08-17 的靜態快照**（998 列全部同一天），從此沒再更新過。跑
> `node tools/fetch-timetable.mjs --dates` 就看得出來。轉檔器現在會照日期擋下
> 來，不會拿三年前的班次覆蓋班表，錯誤訊息會直接說「這個來源不是即時資料」。
>
> 要真的即時資料，得換成
> [TDX 運輸資料流通服務](https://tdx.transportdata.tw/)的 Air FIDS API（免費，
