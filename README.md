...
`data/timetable.json`（見下一節）。**抓不到就是空的**：左下角的銘牌會寫
`NO FEED`，板上一班都不會有。一面掛在牆上的航班看板寧可空著，也不能理
直氣壯地顯示假航班。
...
翻下一頁，翻到最後一頁就回到第一頁（也就是往前那頁），一直繞。**板上不寫第幾頁**
—— 頁碼對站在看板前的人沒有意義。班次會隨時間掉出視窗、頁數跟著變，頁碼每次都會
收斂回實際範圍。等不及 50 秒就按 `Space`。
...
TDX 給的班號常常帶一個尾碼，像 `BR 26D`、`CI 53D`、`JX 2D`。那是 IATA 班機代號規
格裡的 **operational suffix** —— 班機代號的格式是「航空公司代碼 + 1~4 位數字 +
可選的一個字母」，最後那個字母是航空公司用來區分同一個班號的作業變體（換機型、
改航路之類），排班系統和機場之間交換資料（AIDX）時才有意義。

它不是班號的一部分：旅客的機票、訂位系統、機場的旅客看板都只寫數字。TDX 的資料
是從機場 FIDS 來的所以會帶著它，轉檔時直接去掉 —— `BR 26D` 上板就是 `BR 26`。

規則是「結尾是單一英文字母、且前面是 1~4 位數字」才去掉，所以 `IT 216`、`5J 313`
這種不受影響。
...
就是精確值，不必動用 `Intl` 或時區資料庫，看板機器設在哪一區都一樣。大字列下面
那條時鐘也是 TPE 時間。

時鐘那條線用的是 `NUMS`（13 片）而不是 `DRUM`（50 片）：秒數每秒都要翻，用大鼓
的話每一秒都得轉大半圈，吵又費力。`TAIPEI` 那幾個字母不在 `NUMS` 上，會在第一次
寫入時插進那幾格自己的鼓裡，之後就不動了。

打字上板時那條線借去顯示 `CUSTOM MESSAGE`，時鐘先讓位；留空按 Enter 還原。
...
- 每 30 分鐘抓一次 [TDX](https://tdx.transportdata.tw/) 的 Air FIDS，轉成
  `data/timetable.json`：
...
TDX 是交通部的運輸資料平台，免費但要金鑰，而且**按次計算額度**。到
<https://tdx.transportdata.tw/> 註冊 → 會員中心 → 建立應用程式，拿到
`client_id` / `client_secret` 之後放進環境變數：
...
長期跑就寫進系統環境變數。**不要寫進專案檔案** —— 那是憑證。

額度怎麼換算間隔：

```
每次更新 = 1 次呼叫（Air/FIDS/Airport/TPE 一次同時回出發和抵達）
最短間隔 = 1440 分鐘 ÷ 每日額度
...
預設是 30 分鐘（一天約 48 次）。啟動時會把換算出來的每日次數印出來，跟你的額度
對一下就知道夠不夠。額度用完 TDX 會回 429，錯誤訊息會告訴你怎麼調。

**半小時更新一次夠不夠？** 對這個看板夠 —— 狀態是用時鐘算的（見上面的分段表），
不靠即時資料餵。半小時會退化的只有兩件事：登機門可能是半小時前的、誤點最多晚半
小時才反映。班表本身（今天有哪些班、幾點飛）半小時內幾乎不會變。

機場代碼對名稱的表（`Air/Airport`）是靜態資料，抓一次存進 `data/airports.json`，
30 天內不再抓，不佔額度。

看板自己每 2 分鐘回頭抓 `data/timetable.json`，所以**不用重新整理**，班表換了就會
自己翻上去。
...
**桃機掛在政府開放平台上的兩份資料都沒在維護**，別用：

| 資料集 | 頁面宣稱 | 實際 |
| --- | --- | --- |
| [即時航班 26194](https://data.gov.tw/dataset/26194) | 每 5 分更新 | 凍在 **2023-08-17**（998 列全部同一天） |
| [定期航班 7869](https://data.gov.tw/dataset/7869) | 每 7 日更新 | 有效區間到 **2025-11-02** 為止 |

兩個都能用 `node tools/fetch-timetable.mjs <網址> --dates` 自己驗。轉檔器也會照
日期擋下來，不會拿舊班次覆蓋班表 —— 錯誤訊息會直接說「這個來源不是即時資料」。

官網（`www.taoyuan-airport.com/flight_depart`）的班表確實在頁面原始碼裡，但
Cloudflare 是**間歇性**擋的：同一台機器同一個網址，這次過、下次 403。當成每幾分鐘
抓一次的來源不可靠，所以沒走這條。
...
node tools/fetch-timetable.mjs --dates         # 這份資料到底是哪幾天的（定期班表則看有效區間）
