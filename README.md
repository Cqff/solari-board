...
flights: {
  ...
  rows: [
    ...
    // ... 共 16 筆，前 8 筆在左區，後 8 筆在右區
  ]
}
```

版面尺寸在同一個檔案的 `COLS` 和 `PER_BLOCK`：
...
- 葉片**不要圓角** — 圓角會讓繪製器對約 1900 個旋轉元素各做一次遮罩裁切
...
- 翻頁聲踩拍子而非一片一響 — 整面更新約 1500 次翻頁，忠實播放必然是噪音
